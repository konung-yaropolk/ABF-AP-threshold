...
def spike_detect(record, spike_h=0, spike_w=2, spike_d=10, l_lim=False, r_lim=False, lim_adj=15):
    ...
    if not l_lim:
        ...
        if l_diff.size == 1 or r_diff.size == 1:
            raise Exception('There is no AP spikes detected in recording')
        else:
...
def g_t(der_array, der2_array, noise_win=20, noise_tolerance=1):
    ...
    for i in range(0, len(der_array)):
        ...
        # catch the case with few max points
        var = np.where(der_ratio == np.max(der_ratio, 0))
        if len(*var)!=1:
            der_ratio_max.append([np.array([-1])])
        else:
            der_ratio_max.append(var)

    return np.array(der_ratio_list), np.array(der_ratio_max)



def h_t(der, der2, der3, der_array, noise_win=20, noise_tolerance=1):
...
# Data uploading
    ...
    run(res_path, reg_list, save_plot, save_csv, df)




def run(res_path, reg_list, save_plot, save_csv, df):
    ...
    for reg in reg_list:
        ...
        try:
            ...
            g_t_array, g_t_max = g_t(der_array, der2_array,
                                    noise_win=100, noise_tolerance=1)  # 15 noise SD, realy?!
            ...
            for i in range(0, len(voltage_array)):
                ...
                v_th = ' ' if g_t_max[i][0][0] == -1 else round(float(voltage_array[i][th_i]), 3)   # catch the case of multiple max points and place blank cell
